...
int GPIO::waitForEdgeCustom(){
	...
	while(count<=1+ignoreCount){  // ignore the first trigger
		i = epoll_wait(epollfd, &ev, 1, -1);
		if (i==-1){
			perror("GPIO: Poll Wait fail");
			count=100+ignoreCount; // terminate loop
		}
		else {
			count++; // count the triggers up
		}
    ...
    close(fd);
    threadRunning = false;
    if (count==100+ignoreCount) return -1;
	return 0;
}
...
int GPIO::waitForEdge(CallbackType callback, int ignoreCount){
    this->threadRunning = true;
    this->callbackFunction = callback;
    this->ignoreCount = ignoreCount;
// create the thread, pass the reference, address of the function and data
    while(this->threadRunning){
            callbackFunction(this->waitForEdgeCustom());
            usleep(this->debounceTime * 3000);
    }
    this->threadRunning = false;
    return 0;
}
